...
function updateCountdown() {
  ...
  if (deltaTime <= 0) {
    // Якщо час вийшов
    const time = { days: '000', hours: '00', minutes: '00', seconds: '00' };
  ...
  const myday = $(".clock .flipper:nth-child(1) div:not(.new) .text");
  ...
  if ($(myday[0]).text() !== days) {
    flipNumber($(myday[0]).closest(".flipper"), days);
  }
